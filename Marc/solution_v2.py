...
class BO_algo():
    def __init__(self):
        ...
        self.kernel_f = gpytorch.kernels.ScaleKernel(base_kernel=gpytorch.kernels.MaternKernel(nu=kernel_smoothness_f, lengthscale=torch.Tensor([kernel_lenghtscale_f])), outputscale=kernel_variance_f)

        self.kernel_v = gpytorch.kernels.ScaleKernel(base_kernel=gpytorch.kernels.MaternKernel(nu=kernel_smoothness_v, lengthscale=torch.Tensor([kernel_lenghtscale_v])), outputscale=np.sqrt(kernel_variance_v))
    ...
    def next_recommendation(self):
        ...
        return np.atleast_2d(self.optimize_acquisition_function(self))
